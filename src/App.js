...
export default function App() {
  ...
  }, [adoptedIps, scanSubnet, refreshRate, masterServiceIp, masterRefreshRate, configOptions]);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 font-sans">
      ...
      <header className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10 pb-8 border-b border-slate-800">
        ...
        <div className="flex flex-wrap justify-center items-center gap-4">
          {/* Master Link Heartbeat */}
          <div className={`flex items-center gap-3 bg-slate-900/50 px-4 py-2 rounded-xl border transition-all duration-500 shadow-lg ${masterLinkStatus === 'online' ? 'border-emerald-500/30' : 'border-red-500/30'}`}>
            <div className="relative">
              <Server 
                className={`w-5 h-5 transition-all duration-300 ${masterLinkStatus === 'online' ? (isMasterPulsing ? 'text-emerald-500 scale-125' : 'text-emerald-700') : 'text-red-500'}`} 
              />
              {isMasterPulsing && masterLinkStatus === 'online' && (
                <span className="absolute inset-0 animate-ping rounded-full bg-emerald-500/20" />
              )}
              {masterLinkStatus === 'offline' && (
                <AlertTriangle className="absolute -top-1 -right-1 w-2.5 h-2.5 text-red-500 animate-pulse" />
              )}
            </div>
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1 text-nowrap">Master Link</span>
              <span className={`text-xs font-mono leading-none ${masterLinkStatus === 'online' ? 'text-emerald-400' : 'text-red-400 font-bold'}`}>
                {masterLinkStatus === 'online' ? (lastMasterSync || 'Linked') : 'Offline'}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-4 p-4 bg-slate-950 rounded-xl border border-slate-800">
              <div className="flex items-center gap-2 text-blue-400 mb-2">
                ...
                <span className="text-xs font-black uppercase tracking-widest">Timing Engine</span>
              </div>
              <div className="space-y-4">
                ...
                <div className="flex justify-between text-sm pt-2">
                  <span className="text-slate-400 font-medium">Master Link Refresh</span>
                  <span className="text-white font-mono">{masterRefreshRate / 1000}s</span>
                </div>
                <input 
            ...
            <div className="space-y-4 p-4 bg-slate-950 rounded-xl border border-slate-800">
              ...
              <div className="pt-4 space-y-2">
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Master Service IP</span>
                <input 
                  type="text" 
                  value={masterServiceIp}
                  onChange={(e) => setMasterServiceIp(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs font-mono focus:ring-1 focus:ring-emerald-500 outline-none"
            ...
            <div className="space-y-4 p-4 bg-slate-950 rounded-xl border border-slate-800">
              <div className="flex items-center gap-2 text-amber-400 mb-2">
                ...
                <span className="text-xs font-black uppercase tracking-widest">System Control</span>
              </div>
              <div className="space-y-2">
                 ...
                 <button onClick={saveMasterConfig} className="w-full py-2 bg-blue-600 text-white rounded text-[10px] font-bold uppercase flex items-center justify-center gap-2">
                   <Save className="w-3 h-3" /> Save All Configs
                 </button>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 space-y-4 h-[calc(100vh-250px)] overflow-y-auto pr-2 custom-scrollbar">
          ...
          {filteredStations.length === 0 && (
            <div className="py-20 text-center text-slate-600 border border-dashed border-slate-800 rounded-2xl bg-slate-900/20">
              ...
              <button onClick={() => setActiveSettingsPanel('fleet')} className="text-blue-500 hover:text-blue-400 text-xs font-bold mt-4 uppercase">Launch Fleet Manager</button>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-8 bg-slate-900/50 border border-slate-800 rounded-2xl p-6 relative h-[calc(100vh-250px)] overflow-y-auto custom-scrollbar">
          {!selectedStation ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-600 space-y-4 italic">
              ...
              <p>Select a station to begin remote monitoring</p>
            </div>
          ) : (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  ...
                  <span className="text-[10px] text-slate-500 uppercase tracking-tighter">Status Monitoring</span>
                </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className={`text-[10px] font-mono tracking-tighter uppercase ${hasFailed ? 'text-red-500 font-bold' : 'text-slate-500'}`}>
                              {hasFailed ? 'Failed to Restart' : isRestarting ? 'Restarting...' : `${app.status} • ${app.uptime}`}
                            </p>
                          </div>
                        </div>
                        <button 
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl shadow-2xl animate-in zoom-in duration-200 overflow-hidden">
            ...
            <div className="bg-slate-950/50">
              <div className="flex items-center border-b border-slate-800 bg-slate-900/50">
                <button onClick={() => setControlTab('midi')} className={`flex-1 py-4 text-xs font-black uppercase tracking-widest transition-all ${controlTab === 'midi' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>MIDI</button>
                <button onClick={() => setControlTab('osc')} className={`flex-1 py-4 text-xs font-black uppercase tracking-widest transition-all ${controlTab === 'osc' ? 'bg-purple-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>OSC</button>
                <button onClick={() => setControlTab('serial')} className={`flex-1 py-4 text-xs font-black uppercase tracking-widest transition-all ${controlTab === 'serial' ? 'bg-emerald-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>SERIAL</button>
              </div>
                ...
                {controlTab === 'midi' && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in duration-300">
                    ...
                    <div className="flex items-end">
                      <button onClick={() => handleRemoteAction(selectedStation.id, 'midi', controlPayload.midi)} className="w-full bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 transition-all">
                        <Send className="w-4 h-4" /> Trigger
                      </button>
                    </div>
                  </div>
                )}
                {/* OSC and Serial forms follow same structure... */}
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="relative max-w-5xl w-full bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in duration-300">
            <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-900/50">
              <div className="flex items-center gap-2 text-emerald-400 font-bold uppercase tracking-widest text-xs">
                <Camera className="w-4 h-4" /> Exhibit Live Frame
              </div>
              <button onClick={() => setScreenshot(null)} className="p-2 bg-slate-800 hover:bg-red-600 text-slate-300 hover:text-white rounded-lg transition-colors"><X className="w-4 h-4" /></button>
            </div>
            <div className="aspect-video bg-black flex items-center justify-center">
              <img src={screenshot} alt="Exhibit Screenshot" className="max-w-full max-h-full object-contain" />
            </div>
          </div>
        </div>
      )}
...
function StationCard({ station, vitals, isSelected, onClick, smoothing }) {
  return (
    <div 
      onClick={onClick} 
      className={`p-4 rounded-xl border cursor-pointer transition-all active:scale-95 ${isSelected ? 'ring-2 ring-blue-500/50 border-blue-500/50 bg-slate-900 shadow-2xl' : 'border-slate-800 bg-slate-900/40 hover:border-slate-700'}`}
    >
