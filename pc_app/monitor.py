        ...
        if now != last_trigger_min:
            ...
            if at and at == now:
                ...
                for app_entry in config_data.get("watched_apps", []):
                    ...
                    if not app_entry.get("autolaunch", True) or not full_path: continue
...
def health():
    ...
    for p in psutil.process_iter(['name', 'create_time']):
        try:
            if p.info['name'] in watched_names: running_procs[p.info['name']] = p.info
        except: continue
    for app_entry in config_data["watched_apps"]:
        ...
        app_status.append({
            "name": app_name, "path": full_path, "status": "running" if is_running else "stopped", 
            "uptime": uptime, "autolaunch": app_entry.get("autolaunch", True)
        })
    gpu_info = get_gpu_data()
    return jsonify({
        "id": platform.node(), "name": config_data.get("display_name"), "location": config_data.get("location"),
        "status": "online", "vitals": {"cpu": psutil.cpu_percent(), "ram": psutil.virtual_memory().percent, "gpu": gpu_info['load'], "temp": get_system_temp()},
        "apps": app_status, "presets": config_data.get("presets", []),
        "automation": {"autolaunch": config_data.get("autolaunch_time"), "shutdown": config_data.get("shutdown_time"), "enabled": config_data.get("automation_enabled")}
    })
...
def get_screenshot():
    if not HAS_SCREEN_CAPTURE: return jsonify({"error": "Missing libs"}), 500
    try:
        with mss.mss() as sct:
            img = Image.frombytes("RGB", sct.grab(sct.monitors[1]).size, sct.grab(sct.monitors[1]).bgra, "raw", "BGRX")
            img = img.resize((1600, int(img.size[1] * (1600/img.size[0]))), Image.Resampling.LANCZOS)
            buffer = BytesIO()
...
def execute_midi(data):
    ...
    if not HAS_MIDI: return jsonify({"error": "No MIDI"}), 500
    with midi_lock:
        ...
        try:
            if midi_output_obj is None or idx != midi_current_idx:
                if midi_output_obj: midi_output_obj.close()
            ...
            return jsonify({"message": "MIDI sent"})
        except: return jsonify({"error": "MIDI Failed"}), 500

def execute_osc(data):
    if not HAS_OSC: return jsonify({"error": "No OSC"}), 500
    try:
    ...
    except: return jsonify({"error": "OSC Failed"}), 500

def execute_serial(data):
    if not HAS_SERIAL: return jsonify({"error": "No Serial"}), 500
    try:
        ser = serial.Serial(data.get('port') or config_data.get('serial_port', 'COM1'), config_data.get('serial_baud', 9600), timeout=1)
        ser.write(data['message'].encode()); ser.close()
        return jsonify({"message": "Serial sent"})
    except: return jsonify({"error": "Serial Failed"}), 500
...
def trigger_preset():
    preset = next((p for p in config_data["presets"] if p['name'] == request.json.get('name')), None)
    if not preset: return jsonify({"error": "Not found"}), 404
    t = preset.get('type')
    if t == 'midi': return execute_midi(preset)
    elif t == 'osc': return execute_osc(preset)
    elif t == 'serial': return execute_serial(preset)
    return jsonify({"error": "Unknown type"}), 400

@app.route('/action/reboot', methods=['POST'])
def reboot():
    if platform.system() == "Windows": subprocess.Popen("shutdown /r /t 1")
...
def restart_app():
    ...
    path = next((p["path"] for p in config_data["watched_apps"] if os.path.basename(p["path"]) == name), None)
    for p in psutil.process_iter(['name']):
        try:
            if p.info['name'] == name: p.kill()
        except: continue
    if path and os.path.exists(path):
        try:
            if check_requires_admin(path): ctypes.windll.shell32.ShellExecuteW(None, "runas", path, None, None, 1)
...
class MonitorGUI:
    def __init__(self, root):
        ...
        tk.Label(brand_frame, text="EXHIBIT", bg="#1e293b", fg="#3b82f6", font=("Segoe UI", 16, "bold")).pack()
        tk.Label(brand_frame, text="MONITOR", bg="#1e293b", fg="#94a3b8", font=("Segoe UI", 10, "bold")).pack()
        ...
        for text, command in nav_items:
            btn = tk.Button(self.sidebar, text=f"  {text}", command=command, bg="#1e293b", fg="#94a3b8", 
                            activebackground="#334155", activeforeground="white", bd=0, 
                            font=("Segoe UI", 10, "bold"), anchor="w", padx=20, pady=15)
            btn.pack(fill=tk.X)
        ...
        self.page_title = tk.Label(self.header, text="Dashboard", bg="#0f172a", fg="white", font=("Segoe UI", 24, "bold"))
        self.page_title.pack(side=tk.LEFT)

        self.clock_label = tk.Label(self.header, text="00:00:00", bg="#0f172a", fg="#4ade80", font=("Consolas", 16, "bold"))
    ...
    def init_pages(self):
        for name in ["Dashboard", "Identity", "Hardware", "Automation", "App Watcher", "Presets"]:
    ...
    def show_page(self, name):
        ...
        for text, btn in self.nav_buttons.items():
            if text == name: btn.config(bg="#334155", fg="white")
            else: btn.config(bg="#1e293b", fg="#94a3b8")

    def show_dashboard(self): self.show_page("Dashboard")
    def show_identity(self): self.show_page("Identity")
    def show_hardware(self): self.show_page("Hardware")
    def show_automation(self): self.show_page("Automation")
    def show_apps(self): 
    ...
    def setup_dashboard_page(self):
        ...
        self.stat_widgets = {}
        stats = [("CPU USAGE", "cpu"), ("RAM USAGE", "ram"), ("GPU LOAD", "gpu"), ("CORE TEMP", "temp")]
        
        for i, (label, key) in enumerate(stats):
            card = tk.Frame(cards_frame, bg="#1e293b", padx=20, pady=20, width=180, height=120)
            card.grid(row=0, column=i, padx=5, sticky="nsew")
            card.pack_propagate(False)
            tk.Label(card, text=label, bg="#1e293b", fg="#64748b", font=("Segoe UI", 8, "bold")).pack(anchor="w")
            val_label = tk.Label(card, text="--", bg="#1e293b", fg="#3b82f6", font=("Segoe UI", 20, "bold"))
        ...
        tk.Label(status_card, text="AGENT STATUS", bg="#1e293b", fg="white", font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(0, 15))
        ...
        self.create_info_row(grid, "Service Status:", "ACTIVE / LISTENING", 2, fg="#10b981")
        
        actions = tk.Frame(page, bg="#0f172a")
        actions.pack(fill=tk.X, pady=10)
        tk.Button(actions, text="REBOOT SYSTEM", command=reboot, bg="#ef4444", fg="white", bd=0, font=("Segoe UI", 9, "bold"), padx=20, pady=10).pack(side=tk.LEFT)

    def create_info_row(self, parent, label, value, row, fg="#94a3b8"):
        tk.Label(parent, text=label, bg="#1e293b", fg="#64748b", font=("Segoe UI", 10)).grid(row=row, column=0, sticky="w", pady=4)
        tk.Label(parent, text=value, bg="#1e293b", fg=fg, font=("Consolas", 10, "bold")).grid(row=row, column=1, sticky="w", padx=20, pady=4)

    def setup_identity_page(self):
        page = self.pages["Identity"]
        f = tk.Frame(page, bg="#1e293b", padx=30, pady=30)
        f.pack(fill=tk.X, pady=20)
        self.name_entry = self.create_styled_input(f, "Agent Display Name", config_data["display_name"], 0)
        self.loc_entry = self.create_styled_input(f, "Physical Location", config_data["location"], 1)
        tk.Button(page, text="SAVE CHANGES", command=self.save_all_config, bg="#3b82f6", fg="white", bd=0, font=("Segoe UI", 10, "bold"), pady=12).pack(fill=tk.X, pady=20)

    def setup_hardware_page(self):
        page = self.pages["Hardware"]
        f = tk.Frame(page, bg="#1e293b", padx=30, pady=30)
        f.pack(fill=tk.X, pady=20)
        tk.Label(f, text="Default MIDI Output", bg="#1e293b", fg="#64748b", font=("Segoe UI", 10)).grid(row=0, column=0, sticky="w")
        self.midi_cb = ttk.Combobox(f, state="readonly", width=40)
        self.midi_cb.grid(row=0, column=1, sticky="ew", padx=20, pady=15)
        self.serial_entry = self.create_styled_input(f, "Serial Port", config_data["serial_port"], 1)
        self.baud_entry = self.create_styled_input(f, "Baud Rate", str(config_data["serial_baud"]), 2)
        tk.Button(page, text="SAVE HARDWARE CONFIG", command=self.save_all_config, bg="#3b82f6", fg="white", bd=0, font=("Segoe UI", 10, "bold"), pady=12).pack(fill=tk.X, pady=20)

    def setup_automation_page(self):
        page = self.pages["Automation"]
        f = tk.Frame(page, bg="#1e293b", padx=30, pady=30)
        f.pack(fill=tk.X, pady=20)
        self.launch_h, self.launch_m = self.create_styled_time_picker(f, "Daily Startup Time", config_data.get("autolaunch_time", "09:00"), 0)
        self.shutdown_h, self.shutdown_m = self.create_styled_time_picker(f, "Daily Shutdown Time", config_data.get("shutdown_time", "22:00"), 1)
        self.auto_enabled_var = tk.BooleanVar(value=config_data.get("automation_enabled", True))
        tk.Checkbutton(f, text="Enable Automatic Scheduling", variable=self.auto_enabled_var, bg="#1e293b", fg="white", selectcolor="#0f172a", activebackground="#1e293b", pady=20).grid(row=2, column=0, columnspan=2, sticky="w")
        tk.Button(page, text="SAVE AUTOMATION", command=self.save_all_config, bg="#3b82f6", fg="white", bd=0, font=("Segoe UI", 10, "bold"), pady=12).pack(fill=tk.X, pady=20)
    ...
    def setup_apps_page(self):
        ...
        self.app_scrollbar = ttk.Scrollbar(container, orient="vertical", command=self.app_canvas.yview)
        ...
        self.app_canvas.create_window((0, 0), window=self.app_scroll_frame, anchor="nw", width=600)
        ...
        btns.pack(fill=tk.X)
        tk.Button(btns, text="+ ADD APP", command=self.add_app, bg="#10b981", fg="white", bd=0, font=("Segoe UI", 9, "bold"), padx=20, pady=10).pack(side=tk.LEFT)
        tk.Button(btns, text="REMOVE UNCHECKED", command=self.remove_apps, bg="#ef4444", fg="white", bd=0, font=("Segoe UI", 9, "bold"), padx=20, pady=10).pack(side=tk.RIGHT)
    ...
    def setup_presets_page(self):
        ...
        self.preset_list = tk.Listbox(f, bg="#1e293b", fg="white", bd=0, highlightthickness=0, font=("Segoe UI", 10), selectbackground="#334155")
        ...
        tk.Button(btns, text="+ MIDI", command=lambda: self.add_preset_dialog('midi'), bg="#8b5cf6", fg="white", **cfg).pack(side=tk.LEFT, padx=2)
        tk.Button(btns, text="+ OSC", command=lambda: self.add_preset_dialog('osc'), bg="#a78bfa", fg="white", **cfg).pack(side=tk.LEFT, padx=2)
        tk.Button(btns, text="+ SERIAL", command=lambda: self.add_preset_dialog('serial'), bg="#10b981", fg="white", **cfg).pack(side=tk.LEFT, padx=2)
        tk.Button(btns, text="DELETE", command=self.delete_preset, bg="#ef4444", fg="white", **cfg).pack(side=tk.RIGHT, padx=2)

    def create_styled_input(self, parent, label, value, row):
        tk.Label(parent, text=label, bg="#1e293b", fg="#64748b", font=("Segoe UI", 10)).grid(row=row, column=0, sticky="w", pady=5)
        # Fix: Standard Entry does not take 'padx' in constructor. Styling with highlightthickness for 'border' effect.
        e = tk.Entry(parent, bg="#0f172a", fg="white", bd=0, highlightthickness=1, 
    ...
    def create_styled_time_picker(self, parent, label, current_time, row):
        ...
        try: h, m = current_time.split(':')
        except: h, m = "00", "00"
        h_s = ttk.Spinbox(f, from_=0, to=23, format="%02.0f", width=5, font=("Segoe UI", 11))
        h_s.set(h)
        h_s.pack(side=tk.LEFT)
        tk.Label(f, text=":", bg="#1e293b", fg="white", font=("Segoe UI", 12, "bold")).pack(side=tk.LEFT, padx=5)
        m_s = ttk.Spinbox(f, from_=0, to=59, format="%02.0f", width=5, font=("Segoe UI", 11))
        m_s.set(m)
    ...
    def create_tray_icon(self):
        ...
        menu = pystray.Menu(pystray.MenuItem("Restore", self.show_window), pystray.MenuItem("Exit", self.quit_all))
        self.icon = pystray.Icon("exhibit_monitor", image, "Monitor Agent", menu)
        threading.Thread(target=self.icon.run, daemon=True).start()

    def minimize_to_tray(self):
        self.root.withdraw()
        if not HAS_TRAY: self.quit_all()
    ...
    def save_all_config(self):
        ...
        config_data["midi_device_index"] = int(m_sel.split(":")[0]) if ":" in m_sel else -1
        config_data["serial_port"] = self.serial_entry.get()
        try: config_data["serial_baud"] = int(self.baud_entry.get())
        except: pass
        config_data["autolaunch_time"] = f"{int(self.launch_h.get()):02}:{int(self.launch_m.get()):02}"
        config_data["shutdown_time"] = f"{int(self.shutdown_h.get()):02}:{int(self.shutdown_m.get()):02}"
        config_data["automation_enabled"] = self.auto_enabled_var.get()
        for app_entry in config_data["watched_apps"]:
            if hasattr(app_entry, "_var"): app_entry["autolaunch"] = app_entry["_var"].get()
        save_config(config_data)
        messagebox.showinfo("Success", "Settings saved.")

    def refresh_app_list(self):
        for w in self.app_scroll_frame.winfo_children(): w.destroy()
        for a in config_data["watched_apps"]:
            ...
            a["_var"] = v
            tk.Checkbutton(f, variable=v, bg="#1e293b", activebackground="#1e293b", selectcolor="#0f172a").pack(side=tk.LEFT, padx=10)
            lbls = tk.Frame(f, bg="#1e293b")
            lbls.pack(side=tk.LEFT, fill=tk.X)
            tk.Label(lbls, text=os.path.basename(a["path"]), bg="#1e293b", fg="white", font=("Segoe UI", 10, "bold")).pack(anchor="w")
            tk.Label(lbls, text=a["path"], bg="#1e293b", fg="#64748b", font=("Segoe UI", 8)).pack(anchor="w")
    ...
    def remove_apps(self):
        config_data["watched_apps"] = [a for a in config_data["watched_apps"] if a["_var"].get()]
        save_config(config_data)
        self.refresh_app_list()
    ...
    def refresh_presets(self):
        ...
        for p in config_data["presets"]:
            ...
            self.preset_list.insert(tk.END, f" {icon}  {p['name']} ({p['type'].upper()})")

    def delete_preset(self):
        s = self.preset_list.curselection()
        if s:
            config_data["presets"].pop(s[0]); save_config(config_data); self.refresh_presets()
    ...
    def add_preset_dialog(self, p_type):
        ...
        tk.Label(d, text=f"NEW {p_type.upper()} PRESET", bg="#1e293b", fg="#3b82f6", font=("Segoe UI", 12, "bold")).pack(pady=20)
        tk.Label(d, text="Name:", bg="#1e293b", fg="#94a3b8").pack()
        n_e = tk.Entry(d, bg="#0f172a", fg="white", bd=0, insertbackground="white"); n_e.pack(pady=5, padx=20, fill=tk.X)
        if p_type == 'midi':
            tk.Label(d, text="Note / Vel:", bg="#1e293b", fg="#94a3b8").pack()
            f = tk.Frame(d, bg="#1e293b"); f.pack()
            nt_e = tk.Entry(f, width=10, bg="#0f172a", fg="white", bd=0); nt_e.insert(0, "60"); nt_e.pack(side=tk.LEFT, padx=5)
            vl_e = tk.Entry(f, width=10, bg="#0f172a", fg="white", bd=0); vl_e.insert(0, "100"); vl_e.pack(side=tk.LEFT, padx=5)
        elif p_type == 'osc':
            tk.Label(d, text="Path:", bg="#1e293b", fg="#94a3b8").pack()
            pt_e = tk.Entry(d, bg="#0f172a", fg="white", bd=0); pt_e.insert(0, "/trigger"); pt_e.pack(pady=5, padx=20, fill=tk.X)
            tk.Label(d, text="Port / Val:", bg="#1e293b", fg="#94a3b8").pack()
            f = tk.Frame(d, bg="#1e293b"); f.pack()
            pr_e = tk.Entry(f, width=10, bg="#0f172a", fg="white", bd=0); pr_e.insert(0, "8000"); pr_e.pack(side=tk.LEFT, padx=5)
            va_e = tk.Entry(f, width=10, bg="#0f172a", fg="white", bd=0); va_e.insert(0, "1.0"); va_e.pack(side=tk.LEFT, padx=5)
        else:
            tk.Label(d, text="Message:", bg="#1e293b", fg="#94a3b8").pack()
            ms_e = tk.Entry(d, bg="#0f172a", fg="white", bd=0); ms_e.pack(pady=5, padx=20, fill=tk.X)
        def save():
            new_p = {"name": n_e.get(), "type": p_type}
            if p_type == 'midi': new_p.update({"note": nt_e.get(), "velocity": vl_e.get(), "channel": 1})
            elif p_type == 'osc': new_p.update({"path": pt_e.get(), "port": pr_e.get(), "value": va_e.get()})
            else: new_p.update({"message": ms_e.get()})
            config_data["presets"].append(new_p); save_config(config_data); self.refresh_presets(); d.destroy()
        tk.Button(d, text="SAVE", command=save, bg="#10b981", fg="white", font=("Segoe UI", 9, "bold"), bd=0, pady=10).pack(pady=30, padx=20, fill=tk.X)
